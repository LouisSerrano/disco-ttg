...
def _count_flops(wrapper_module, *inputs):
    """Helper to compute FLOPs for a given wrapper module."""
    with torch.no_grad():
        analysis = FlopCountAnalysis(wrapper_module, inputs if len(inputs) > 1 else inputs[0])
...
def test_direct_prediction(model, dataloader, dt=4/250):
    ...
    for batch in dataloader:
        ...
        with torch.no_grad():
            # Skip encoding FLOPs due to tensor shape incompatibility with FLOP counter
            encoding_flops = 0
            total_flops += encoding_flops

            # Count FLOPs using same approach as random method - avoid JIT issues
            dim = 1 if len(inp.shape) == 4 else 2
            state_sample = state_labels if state_labels.ndim == 1 else state_labels[:1]
            opnn_flops_per_sample = _count_flops(model.opnns[str(dim)], inp[:1, 0].detach(), state_sample.detach())
            fe_multiplier = 4 if getattr(model, "solver", None) == "rk4" else 2 if getattr(model, "solver", None) == "midpoint" else 1

            # Get prediction and compute error on future frames
            pred, metadata = model(inp, state_labels, n_future_steps=target.shape[1])
            steps = metadata.get('n_steps', 1)
            prediction_flops = opnn_flops_per_sample * steps * inp.shape[0] * fe_multiplier
            total_flops += prediction_flops
            predictions.append(pred.cpu())
            error = relative_l2_error(pred, target).item()
            total_error += error * inp.shape[0]
            samples += inp.shape[0]
            
            # Calculate prediction loss on observed trajectories (one-step ahead prediction within input sequence)
            x_val = rearrange(inp[:, :-1], "b t ... -> (b t) ...")
            y_val = rearrange(inp[:, 1:], "b t ... -> (b t) ...")
            
            # Get one-step predictions on observed trajectory using direct model
            obs_metadata = {}
            obs_pred, obs_metadata = model(x_val.unsqueeze(1), state_labels, n_future_steps=1)
            #obs_steps = obs_metadata.get('n_steps', 1)
            #obs_flops = opnn_flops_per_sample * obs_steps * inp[:, :-1].shape[0] * fe_multiplier
            #total_flops += obs_flops
            obs_pred_flat = rearrange(obs_pred, "b t ... -> (b t) ...")
            obs_loss = relative_l2_error(obs_pred_flat, y_val).item()
            prediction_losses.append(obs_loss)
    ...
    avg_flops = total_flops / samples  # Average FLOPs per sample
    return avg_error, torch.cat(predictions), avg_flops, total_time, avg_prediction_loss
...
def greedy_operator_selection(model, theta_latent_operators, inp, target, max_operators=5, min_improvement_threshold=5.0, dt=4/250):
    ...
    for step in range(max_operators):
        ...
        for op_idx in range(theta_latent_operators.shape[0]):
            ...
            if len(test_latent_operators) == 1:
                # Count FLOPs for single operator
                class SingleOpWrapper(torch.nn.Module):
                    def __init__(self, model, x_val, test_operators, state_labels, dim, dt):
                        super().__init__()
                        self.model = model
                        self.x_val = x_val
                    ...
                    def forward(self, dummy_input):
                        pred, _ = self.model.solve_ode(self.x_val, self.test_operators, self.state_labels,
                                                     dim=self.dim, integration_time=self.dt, n_future_steps=1, dt=self.dt)
                        return pred
                
                wrapper = SingleOpWrapper(model, x_val, test_operators[0], state_labels, dim, dt)
                dummy_input = torch.zeros(1, device=x_val.device)
                flops = _count_flops(wrapper, dummy_input)
                total_flops += flops
                ...
                # Count FLOPs for multi-operator composition
                class MultiOpWrapper(torch.nn.Module):
                    def __init__(self, model, test_latent_operators, x_val, dt):
                        super().__init__()
                        self.model = model
                        self.test_latent_operators = test_latent_operators
                        self.x_val = x_val
                        self.dt = dt
                    
                    def forward(self, dummy_input):
                        pred = multi_operator_splitting(self.model, self.test_latent_operators, self.x_val, 
                                                       nt=1, dt=self.dt)
                        return pred
                
                wrapper = MultiOpWrapper(model, test_latent_operators, x_val, dt)
                dummy_input = torch.zeros(1, device=x_val.device)
                flops = _count_flops(wrapper, dummy_input)
                total_flops += flops
    ...
    with torch.no_grad():
        ...
        if len(current_composition) > 1:
            ...
            # Count FLOPs for final multi-operator prediction
            class FinalMultiOpWrapper(torch.nn.Module):
                def __init__(self, model, best_latent_operators, inp, target_shape, dt):
                    super().__init__()
                    self.model = model
                    self.best_latent_operators = best_latent_operators
                    self.inp = inp
                    self.target_shape = target_shape
                    self.dt = dt
                
                def forward(self, dummy_input):
                    pred = multi_operator_splitting(self.model, self.best_latent_operators, self.inp[:, -1], 
                                                   nt=self.target_shape[1], dt=self.dt)
                    return pred
            
            wrapper = FinalMultiOpWrapper(model, best_latent_operators, inp, target.shape, dt)
            dummy_input = torch.zeros(1, device=inp.device)
            flops = _count_flops(wrapper, dummy_input)
            total_flops += flops
            ...
            # Count FLOPs for final single operator prediction
            class FinalSingleOpWrapper(torch.nn.Module):
                def __init__(self, model, inp, best_operators, state_labels, dim, dt, target_shape):
                    super().__init__()
                    self.model = model
                    self.inp = inp
                    self.best_operators = best_operators
                    self.state_labels = state_labels
                    self.dim = dim
                    self.dt = dt
                    self.target_shape = target_shape
                ...
                def forward(self, dummy_input):
                    pred, _ = self.model.solve_ode(self.inp[:, -1], self.best_operators, self.state_labels,
                                                  dim=self.dim, integration_time=self.dt, n_future_steps=self.target_shape[1], dt=self.dt)
                    return pred
            
            wrapper = FinalSingleOpWrapper(model, inp, best_operators, state_labels, dim, dt, target.shape)
            dummy_input = torch.zeros(1, device=inp.device)
            flops = _count_flops(wrapper, dummy_input)
            total_flops += flops
    ...
    total_time = time.perf_counter() - start_time
    return best_composition, best_error, best_pred, total_flops, total_time
...
def random_operator_selection(model, theta_latent_operators, inp, target, num_compositions=100, composition_lengths=[2], dt=4/250):
    ...
    dim = 1 if len(inp.shape) == 4 else 2
    # Per-sample opnn FLOPs (used to scale by actual batch and solver steps)
    state_sample = state_labels if state_labels.ndim == 1 else state_labels[:1]
    opnn_flops_per_sample = _count_flops(model.opnns[str(dim)], x_val[:1].detach(), state_sample.detach())
    fe_multiplier = 4 if getattr(model, "solver", None) == "rk4" else 2 if getattr(model, "solver", None) == "midpoint" else 1
    ...
    for _ in range(num_compositions):
        ...
        if len(test_latent_operators) == 1:
            with torch.no_grad():
                ...
                meta = {}
                pred, meta = model.solve_ode(x_val, test_operators, state_labels,
                                        dim=dim, integration_time=dt, n_future_steps=1, dt=dt, metadata=meta)
                steps = meta.get('n_steps', 0)
                total_flops += opnn_flops_per_sample * steps * x_val.shape[0] * fe_multiplier
        else:
            with torch.no_grad():
                pred, steps = multi_operator_splitting(model, test_latent_operators, x_val,
                                                     nt=1, dt=dt, return_steps=True)
                total_flops += opnn_flops_per_sample * steps * x_val.shape[0] * fe_multiplier
    ...
    if len(best_composition) > 0:
        with torch.no_grad():
            if len(best_composition) > 1:
                pred, steps = multi_operator_splitting(model, best_latent_operators,
                                                       inp[:, -1], nt=target.shape[1], dt=dt, return_steps=True)
                total_flops += opnn_flops_per_sample * steps * inp[:, -1].shape[0] * fe_multiplier
            else:
                best_operators = model.decode_theta(best_latent_operators[0], dim)
                metadata = {}
                pred, metadata = model.solve_ode(inp[:, -1], best_operators, state_labels,
                                        dim=dim, integration_time=dt, n_future_steps=target.shape[1], dt=dt, metadata=metadata)
                steps = metadata.get('n_steps', 0)
                total_flops += opnn_flops_per_sample * steps * inp[:, -1].shape[0] * fe_multiplier
    else:
    ...
    total_time = time.perf_counter() - start_time
    return best_composition, test_error, best_pred, total_flops, total_time
...
def random_operator_selection_batch(model, theta_latent_operators, inp, target, num_compositions=128, composition_lengths=[2], dt=4/250, random_batch_size=32):
    ...
    start_time = time.perf_counter()
    total_flops = 0
    # Running stats local to this call (used by caller for global aggregation)
    trials_this_call = 0
    flops_sum_this_call = 0.0
    error_sum_this_call = 0.0
    flops_running_avg_local = []
    error_running_avg_local = []
    ...
    # Per-sample opnn FLOPs (used to scale by actual batch and solver steps)
    state_sample = state_labels if state_labels.ndim == 1 else state_labels[:1]
    opnn_flops_per_sample = _count_flops(model.opnns[str(1 if len(inp.shape) == 4 else 2)],
                                         x_val[:1].detach(), state_sample.detach())
    fe_multiplier = 4 if getattr(model, "solver", None) == "rk4" else 2 if getattr(model, "solver", None) == "midpoint" else 1
    ...
    for _ in range(0, num_compositions, random_batch_size):
        ...
        if length == 1:
            with torch.no_grad():
                ...
                metadata = {}
                pred, metadata = model.solve_ode(x_val, test_operators, state_labels,
                                        dim=dim, integration_time=dt, n_future_steps=1, dt=dt, metadata=metadata)
            ...
            with torch.no_grad():
                pred, steps = multi_operator_splitting(model, test_latent_operators_batch, x_val,
                                                     nt=1, dt=dt, return_steps=True)
                pred = rearrange(pred, '(b t) ... -> b t ...', b=random_batch_size)
                total_flops += opnn_flops_per_sample * steps * x_val.shape[0] * fe_multiplier

        errors = [relative_l2_error(pred[k], y_val).item() for k in range(random_batch_size)]
        trials_this_call += len(errors)
        flops_sum_this_call = total_flops
        error_sum_this_call += sum(errors)
        
        # Track trajectory of total FLOPs and best error so far
        ...
        for error in errors:
            flops_running_avg_local.append(total_flops)
            error_running_avg_local.append(best_error)

    # Final prediction on full sequence
    if len(best_composition) > 0:
        with torch.no_grad():
            if len(best_composition) > 1:
                pred, steps = multi_operator_splitting(model, best_latent_operators, inp[:, -1],
                                                       nt=target.shape[1], dt=dt, return_steps=True)
                total_flops += opnn_flops_per_sample * steps * inp[:, -1].shape[0] * fe_multiplier
            else:
                best_operators = model.decode_theta(best_latent_operators[0], dim)
                metadata = {}
    ...
    total_time = time.perf_counter() - start_time
    return best_composition, test_error, best_pred, total_flops, total_time, flops_running_avg_local, error_running_avg_local, trials_this_call, flops_sum_this_call, error_sum_this_call
...
def multi_operator_splitting(model, theta_latents, x, nt=1, dt=4/250, refinement_factor=1, splitting_method="strang", return_steps=False):
        ...
        for ref_step in range(refinement_factor):
            ...
            if splitting_method == 'strang' and k_operators > 1:
                ...
                for i in range(k_operators - 1):
                    meta = {}
                    pred, meta = model.solve_ode(pred, thetas[i], state_labels, dim, 
                                            n_future_steps=1, integration_time=small_dt/2, dt=small_dt/2, metadata=meta)
                    total_steps += meta.get('n_steps', 0)
                    pred = pred[:, -1]
                
                # Full step for last operator
                meta = {}
                pred, meta = model.solve_ode(pred, thetas[-1], state_labels, dim, 
                                        n_future_steps=1, integration_time=small_dt, dt=small_dt, metadata=meta)
                total_steps += meta.get('n_steps', 0)
                pred = pred[:, -1]
                ...
                for theta in thetas:
                    meta = {}
                    pred, meta = model.solve_ode(pred, theta, state_labels, dim, 
                                            n_future_steps=1, integration_time=small_dt, dt=small_dt, metadata=meta)
                    total_steps += meta.get('n_steps', 0)
                    pred = pred[:, -1]
    ...
    if return_steps:
        return trajectory_pred, total_steps
...
def beam_search_operator_selection_batch(model, theta_latent_operators, inp, target,
        ...
        for batch_start in range(0, len(candidates), batch_size):
            ...
            for length, group in length_groups.items():
                if length == 1:
                    ...
                    with torch.no_grad():
                        metadata = {}
                        pred, metadata = model.solve_ode(x_val_batch[:len(group)*original_shape[0]],
                                                group_ops, state_labels,
                                                dim=dim, integration_time=dt,
                                                n_future_steps=1, dt=dt, metadata=metadata)
                        pred = rearrange(pred, '(b t) ... -> b t ...', b=len(group))
                        
                        # Count FLOPS using the same approach as random method
                        steps = metadata.get('n_steps', 1)
                        batch_flops = opnn_flops_per_sample * steps * len(group) * original_shape[0] * fe_multiplier
                    ...
                    with torch.no_grad():
                        pred, steps = multi_operator_splitting(model, group_latent_ops,
                ...
                # Calculate errors for this group and track trajectories
                for j, (batch_idx, _, _, _) in enumerate(group):
                    ...
                    # Track this trial
                    trials_this_call += 1
                    flops_sum_this_call += total_flops / trials_this_call if trials_this_call > 0 else 0
                    error_sum_this_call += error
                    
                    # Update trajectory tracking
                    flops_trajectory_local.append(total_flops)
                    current_best_error = min(_best_overall_error, error)
                    if current_best_error < _best_overall_error:
                        _best_overall_error = current_best_error
                    error_trajectory_local.append(_best_overall_error)
    ...
    if len(best_overall_composition) > 0:
        with torch.no_grad():
            ...
            if len(best_overall_composition) > 1:
                ...
                pred, metadata = model.solve_ode(inp[:, -1], best_operators, state_labels,
                                                dim=dim, integration_time=dt,
                                                n_future_steps=target.shape[1], dt=dt, metadata=metadata)
                steps = metadata.get('n_steps', 0)
                total_flops += opnn_flops_per_sample * steps * inp[:, -1].shape[0] * fe_multiplier
    ...
    # Calculate final timing and return trajectory information
    total_time = time.perf_counter() - start_time
    
    return (best_overall_composition, test_error, best_overall_pred, total_flops, total_time,
            flops_trajectory_local, error_trajectory_local, trials_this_call, 
            flops_sum_this_call, error_sum_this_call)
