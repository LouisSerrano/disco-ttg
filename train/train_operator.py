...
import wandb
from src.operators.disco_operator import DiscoOperator, disco_operator_mini
...
class DISCOOperatorLitModule(L.LightningModule):
    def __init__(self, model_cfg, training_cfg):
        super().__init__()
        self.save_hyperparameters()
        self.loss_fn = RelativeL2()
        self.model = DiscoOperator(**model_cfg)
        #self.model = disco_operator_mini(**model_cfg)
    ...
    def training_step(self, batch, batch_idx):
        input = rearrange(batch['context_input'], 'b t c h -> b c h t') if self.in_context else rearrange(batch['input'], 'b t c h -> b c h t')
        target = rearrange(batch['target'], 'b t c h -> b c h t') 
        ...
        optimizer.zero_grad()
        y_pred, conditioning = self.model(input, y=target[..., :-1])
    ...
    def validation_step(self, batch, batch_idx):
        input = rearrange(batch['input'], 'b t c h -> b c h t')
        target = rearrange(batch['target'], 'b t c h -> b c h t')
        state_labels = torch.tensor([0], device=input.device)
        y_pred, conditioning = self.model(input, y=target[..., :-1])
    ...
    def configure_optimizers(self):
        ...
        warmup_steps = getattr(self, 'warmup_steps', int(0.01 * self.max_steps))
        warmup_steps = int(0.01 * self.max_steps) if warmup_steps is None else warmup_steps
...
def get_run_name(cfg: DictConfig) -> str:
    ...
    model_params = [
        f"h{cfg.model.hidden_channels}",
        f"code{cfg.model.code_dim}",
    ]
    ...
    # Combine all parts
    return f"DISCOOperator_{dataset_name}_{'_'.join(model_params)}_{'_'.join(train_params)}_{'_'.join(data_params)}"

@hydra.main(config_path="../configs", config_name="operator")
def main(cfg: DictConfig):
    ...
    model = DISCOOperatorLitModule(cfg.model, cfg.training)
