...
import wandb
from src.operators.disco_ablations import DiscoAblations
...
class DISCOAblationsLitModule(L.LightningModule):
    def __init__(self, model_cfg, training_cfg):
        super().__init__()
        self.save_hyperparameters()
        self.loss_fn = RelativeL2()
        self.model = DiscoAblations(**model_cfg)
    ...
    def training_step(self, batch, batch_idx):
        ...
        input = batch['input']# if self.in_context else batch['input']
...
def get_run_name(cfg: DictConfig) -> str:
    ...
    # Get model parameters - using ablations prefix to distinguish
    model_params = [
        "ablations",
...
def main(cfg: DictConfig):
    ...
    model = DISCOAblationsLitModule(cfg.model, cfg.training)
