...
import wandb
from src.operators.disco_vae import DISCOHouse
...
class DISCOLitModule(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_pred, metadata = self.model(input, state_labels, y=target_inp, n_future_steps=1, use_vae=True)
        rel_loss = self.loss_fn(y_pred, target_out)
        # VAE loss
        vae_loss = metadata['vae_kl_loss']
        #vae_loss = metadata['vae_kl_loss']

        # Total loss
        loss = rel_loss + 1e-4*vae_loss #0.01 * vae_loss  # Adjust weight as needed
        ...
        self.log('train_kl_loss', vae_loss, on_step=True, on_epoch=True, prog_bar=True)
...
@hydra.main(config_path="../configs", config_name="config_vae_hdf5")
