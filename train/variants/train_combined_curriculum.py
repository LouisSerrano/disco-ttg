...
class DISCOLitModule(L.LightningModule):
    def __init__(self, model_cfg, training_cfg):
        ...
        self.curriculum_enabled = getattr(self, 'curriculum_enabled', False)
        if self.curriculum_enabled:
            self.curriculum_start_frames = getattr(self, 'curriculum_start_frames', 1)
            self.curriculum_end_frames = getattr(self, 'curriculum_end_frames', 16)
            self.curriculum_warmup_steps = getattr(self, 'curriculum_warmup_steps', int(0.5 * self.max_steps))
            print(f"Curriculum learning enabled:")
            print(f"  Start frames: {self.curriculum_start_frames}")
            print(f"  End frames: {self.curriculum_end_frames}")
            print(f"  Warmup steps: {self.curriculum_warmup_steps}")
        
        # Track current target frames for curriculum
        self.current_target_frames = self.curriculum_start_frames if self.curriculum_enabled else None
    ...
    def _get_current_target_frames(self, global_step):
        """Get the current number of target frames based on curriculum schedule."""
        if not self.curriculum_enabled:
            return None  # Use all available frames
        
        if global_step >= self.curriculum_warmup_steps:
            return self.curriculum_end_frames
        
        # Linear interpolation
        progress = global_step / self.curriculum_warmup_steps
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Update target frames based on curriculum
        if self.curriculum_enabled:
            self.current_target_frames = self._get_current_target_frames(self.global_step)
        ...
        #target = batch['target_output'] if self.in_context else batch['output']
        
        # Apply curriculum learning by potentially truncating the target sequence
        #if self.current_target_frames is not None and self.current_target_frames < target.shape[1]:
            # Randomly sample a starting point to ensure we train on different parts
        #    max_start = target.shape[1] - self.current_target_frames
        #    start_idx = np.random.randint(0, max_start + 1)
        #    target = target[:, start_idx:start_idx + self.current_target_frames]
        ...
        optimizer.zero_grad()
        y_pred, metadata = self.model(input, state_labels, n_future_steps=self.current_target_frames)
        loss = self.loss_fn(y_pred, target[:, :self.current_target_frames])
        ...
        if self.curriculum_enabled:
            self.log('curriculum_frames', self.current_target_frames, on_step=True, prog_bar=True)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # During validation, we can test on different sequence lengths
        # to see how well the model generalizes
        val_losses = {}
        
        # Test on different target frame counts
        test_frames = [1, 2, 4, 8, 16] if self.curriculum_enabled else [target.shape[1] - 1]
        y_pred, metadata = self.model(input, state_labels, n_future_steps=test_frames[-1])
        
        for n_frames in test_frames:
            if n_frames >= target.shape[1]:
                continue
            
            loss = self.loss_fn(y_pred[:, :n_frames], target[:, :n_frames])
            val_losses[f'val_loss_{n_frames}frames'] = loss
        
        val_losses['val_loss'] = val_losses[f'val_loss_1frames']
        
        # Log all validation losses
        self.log_dict(val_losses, on_step=True, on_epoch=True, prog_bar=False)
...
def get_run_name(cfg: DictConfig) -> str:
    ...
    # Add curriculum parameters if enabled
    if hasattr(cfg.training, 'curriculum_enabled') and cfg.training.curriculum_enabled:
        train_params.append(f"curriculum{cfg.training.curriculum_start_frames}-{cfg.training.curriculum_end_frames}")
...
def main(cfg: DictConfig):
    ...
    # Log curriculum metrics if enabled
    if hasattr(cfg.training, 'curriculum_enabled') and cfg.training.curriculum_enabled:
        wandb_logger.experiment.define_metric("curriculum_frames", step_metric="trainer/global_step")
        for n_frames in [1, 2, 4, 8, 16]:
            wandb_logger.experiment.define_metric(f"val_loss_{n_frames}frames", step_metric="trainer/global_step")
