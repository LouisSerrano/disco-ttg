...
class DISCOLitModule(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if use_vq and self.fsq_monitor is not None:
         ...
         #theta1, _ = self.model.encode_theta_latent(input, state_labels, use_vq=False)
        #theta1 = metadata['theta_latent_pre_vq']
        theta1, _ = self.model.encode_theta_latent(input, state_labels, use_vq=False)
        theta2, _ = self.model.encode_theta_latent(batch['target_input'], state_labels, use_vq=False)

        consistency_loss = ((theta1 - theta2)**2).mean()        

        loss = rel_loss + metadata['vq_loss'] + 0.1*consistency_loss
