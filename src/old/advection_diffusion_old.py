...
class Sine(ShiftedPattern):
    ...
    def generate_pattern(self, batch_size: int, seed: int | None):
        ...
        phase = torch.linspace(0, 2*torch.pi*self.periods, self.patch_size)
...
if __name__ == "__main__":
    ...
    # IC
    initial_condition = Fractaloid(degree=spatial_size, power=2.0, size=spatial_size, patch_size=spatial_size)
    u0 = initial_condition.generate(batch_size, None)[:,None,:]

    # operator
    operator = AdvectionDiffusion(velocity=(1,), diffusivity=0.1)

    ut = operator(torch.arange(time_steps), u0, seed=None)
