...
class Hypernetwork(nn.Module):
    ...
    def __init__(self, 
        ...
        # processor (transformer blocks instead of SpaceTimeBlock)
        self.blocks = nn.ModuleList([
...
class DiscoAblations(nn.Module):
    ...
    def __init__(self, 
        ...
        self.hpnn = DiscoEncoder(
            grid_size=256,
            patch_size=patch_size,
            in_chans=n_states,
            history=16,
            embed_dim=hidden_dim,
            depth=processor_blocks,
            num_heads=num_heads,
            mlp_ratio=4.0,
            norm_layer=partial(nn.LayerNorm, eps=1e-6)
        )
        # Feature projection from encoder to theta_dim (like in DiscoOperator)
        self.hpnn_projector = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim*2),
            nn.GELU(),
            nn.Linear(hidden_dim*2, theta_dim)
        )

        # Operator network (opnn)
        self.opnns = nn.ModuleDict({
            str(dim): OperatorNetwork(n_states, 8, dim, boundary_conditions="None", norm_groups=4) 
            for dim in ndims
        })
    ...
    def encode_theta_latent(self, x: Tensor, state_labels: Tensor) -> Tensor:
        ...
        if self.use_standardization:
            ...
            metadata = {'mean': torch.zeros_like(x[:, :1]), 'std': torch.ones_like(x[:, :1])}

        # Estimate the PDE's intrisic parameters
        # Reshape for DiscoEncoder: (B, C, H, T)
        x_encoder = rearrange(x, 'b t c h -> b c h t')
        features = self.hpnn(x_encoder)  # (B, num_patches + 1, embed_dim)
        theta_latent = features[:, 0, :]  # Extract CLS token
...
class DiscoAblationsUNet(nn.Module):
    """ A DISCO model using conditional UNet from disco_operator for NeuralODE. """
    def __init__(self, 
        n_states: int,
        hidden_dim: int,
        patch_size: int,
        ...
        is_attn: tuple = (False, False, False, False),
        mid_attn: bool = False,
        n_blocks: int = 2,
        ...
        # Hypernetwork (hpnn) - using DiscoEncoder
        self.hpnn = DiscoEncoder(
            grid_size=256,
            patch_size=patch_size,
            in_chans=n_states,
        ...
        # Feature projection from encoder to conditioning code
        self.hpnn_projector = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim*2),
            nn.GELU(),
        ...
        # Conditional UNet operator for each dimension
        self.unets = nn.ModuleDict({
            str(dim): Unet(
                n_input_scalar_components=n_states,
                n_input_vector_components=0,
                n_output_scalar_components=n_states,
    ...
    def encode_conditioning(self, x: Tensor, state_labels: Tensor) -> Tensor:
        """Encode input x to produce conditioning code."""
        x_shape = x.shape
        B, T, C = x_shape[:3]
        spatial = x_shape[3:]
        dim = len(spatial)
        ...
        # Encode using DiscoEncoder: (B, C, H, T)
        x_encoder = rearrange(x, 'b t c h -> b c h t')
        features = self.hpnn(x_encoder)  # (B, num_patches + 1, embed_dim)
        conditioning = features[:, 0, :]  # Extract CLS token
        conditioning = self.hpnn_projector(conditioning)
        return conditioning, metadata

    def solve_ode(self, x_input: Tensor, conditioning: Tensor, state_labels: Tensor, dim: int, 
                  integration_time: float, n_future_steps: int, predict_normed: bool, 
                  metadata: dict, dt: float = None, solver: Optional[str] = None) -> Tensor:
        """Integrate using conditional UNet as the neural operator in NeuralODE."""
        B = x_input.shape[0]
        state_labels = state_labels.unsqueeze(0).repeat(B, 1)

        def unet_opnn(t, x):
            # x shape: (B, C, H) for 1D case
            # Reshape conditioning for UNet: repeat for each batch element
            cond = conditioning  # (B, code_dim)
            x_out = self.unets[str(dim)](x, z=cond)
            if x_out.isnan().any():
                print(f"x is nan in unet_opnn")
        ...
        if self.use_adjoint:
            if dt is None:
                dt = integration_time / n_future_steps
            t = torch.arange(0, integration_time + dt, dt, device=x_input.device)
            n_steps, x = odeint_adjoint(unet_opnn, x_input, t=t, rtol=self.rtol, method=current_solver, options=options, adjoint_params=list(self.unets[str(dim)].parameters()))
            x = x[1:,...]
        else:
            if dt is None:
                dt = 1/self.max_steps
            options['step_size'] = dt
            t = torch.arange(0, integration_time + dt, dt, device=x_input.device)
            n_steps, x = odeint(unet_opnn, x_input, t=t, rtol=self.rtol, method=current_solver, options=options)
            x = x[1:,...]
            # Sample at the desired output times
            output_dt = integration_time / n_future_steps
            output_indices = [int(round(i * output_dt / dt)) for i in range(1, n_future_steps + 1)]
    ...
    def forward(self, 
        ...
        # Use initial condition for the solver
        spatial_dims = tuple(range(3,x.ndim))
        x_input = x[:,-1,...]
        ...
        conditioning, metadata = self.encode_conditioning(x, state_labels)
        metadata['conditioning'] = conditioning

        # Helper to use UNet as neural operator
        def unet_opnn(t, x):
            # x shape: (B, C, H) for 1D case  
            x_out = self.unets[str(dim)](x, z=conditioning)
            if x_out.isnan().any():
                print(f"x is nan in unet_opnn")
                print(f"conditioning {conditioning}")
                print(f"x {x}")
