...
from ..modules.vq import ResidualVQ
from vector_quantize_pytorch import FSQ, LFQ, SimVQ
...
class DISCOHouse(nn.Module):
    """DISCO model with VAE for theta latent encoding (no quantization)"""
    def __init__(self, 
        n_states: int,
        hidden_dim: int,
        theta_dim: int,
        ...
        # VAE components for theta latent
        # Encoder: theta_latent -> latent distribution
        self.vae_encoder = nn.Sequential(
           # nn.Linear(theta_dim, theta_dim),
            #nn.ReLU(),
            #nn.Linear(theta_dim, theta_dim),
            #nn.ReLU(),
            nn.Linear(theta_dim, vae_latent_dim * 2)  # mu and logvar
        )
        
        # Decoder: latent -> theta_latent
        self.vae_decoder = nn.Sequential(
            nn.Linear(vae_latent_dim, theta_dim),
            #nn.ReLU(),
            #nn.Linear(theta_dim, theta_dim),
            #nn.ReLU(),
            #nn.Linear(theta_dim, theta_dim)
        ...
        self.opnns = nn.ModuleDict({
            str(dim): OperatorNetwork(n_states, self.opnn_channels, dim, 
                                    boundary_conditions=self.boundary_conditions, 
                                    norm_groups=4, 
                                    bottleneck_multiplier=self.opnn_bottleneck_multiplier)
            for dim in ndims
        })
        
        # Initialize theta norms
    ...
    def apply_principled_initialization(self):
        ...
        for dim_str, decoder_layer in self.decoder_head.items():
                        ...
                        for param_name, module_param in module.named_parameters():
                            if module_param is param:
                                if isinstance(module, nn.Linear):
                                    ...
                                    dj = module.in_channels * param[0,0].numel()
                                else:
                                    dj = 1
                                break
                    variance = 1.0 / (dj * dk)
            ...
            with torch.no_grad():
                decoder_weight = decoder_layer.weight
                param_variances = torch.tensor(variances, device=decoder_weight.device, dtype=decoder_weight.dtype)
                param_stds = param_variances.sqrt()
                random_weights = torch.randn_like(decoder_weight)
                decoder_weight.data = random_weights * param_stds.unsqueeze(1)

    def vae_encode(self, theta_latent):
        """Encode theta_latent to VAE latent distribution"""
        h = self.vae_encoder(theta_latent)
    ...
    def vae_decode(self, z):
        """Decode VAE latent to theta_latent"""
        return self.vae_decoder(z)

    def vae_loss(self, theta_latent, recon_theta_latent, mu, logvar):
        """Compute VAE loss components"""
        # Reconstruction loss
        recon_loss = F.mse_loss(recon_theta_latent, theta_latent)
        
        # KL divergence loss
        kl_loss = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
        kl_loss = kl_loss / theta_latent.size(0)  # Average over batch
        
        return recon_loss, kl_loss

    def encode_theta_latent(self, x: Tensor, state_labels: Tensor, 
                          return_vae_details: bool = False, use_vae: bool = True) -> Tensor:
        """Encode input to theta_latent with optional VAE"""
        x_shape = x.shape
        B, T, C = x_shape[:3]
        spatial = x_shape[3:]
        dim = len(spatial)
        ...
        # Get theta latent from hypernetwork (same as original DISCO)
        ...
        # Store original theta latent
        theta_latent_original = theta_latent
        
        if use_vae:
            # VAE encoding
            mu, logvar = self.vae_encode(theta_latent)
            
            # Reparameterization
            z = self.vae_reparameterize(mu, logvar)
            
            # VAE decoding
            theta_latent_recon = self.vae_decode(z)
            
            # Compute VAE losses
            recon_loss, kl_loss = self.vae_loss(theta_latent, theta_latent_recon, mu, logvar)
            
            # Use the VAE latent (z) as the final theta_latent
            # This forces the model to work in the regularized VAE latent space
            theta_latent = theta_latent_recon
            
            # Store VAE components in metadata
            metadata['vae_recon_loss'] = recon_loss
            metadata['vae_kl_loss'] = kl_loss
            metadata['vae_total_loss'] = recon_loss + self.vae_beta * kl_loss
            metadata['theta_latent_recon'] = theta_latent_recon
            metadata['vae_mu'] = mu
            metadata['vae_logvar'] = logvar
            metadata['vae_z'] = z
        else:
            # No VAE - just pass through original theta_latent
            metadata['vae_recon_loss'] = torch.tensor(0.0, device=x.device)
            metadata['vae_kl_loss'] = torch.tensor(0.0, device=x.device)
            metadata['vae_total_loss'] = torch.tensor(0.0, device=x.device)
        
        if return_vae_details:
            metadata['theta_latent_original'] = theta_latent_original
    ...
    def forward(self, 
        ...
        return_vae_details: bool = False,
        use_vae: bool = True,
    ) -> Tensor:
        """Forward pass with VAE regularization"""
        x_shape = x.shape
        B, T, C = x_shape[:3]
        spatial = x_shape[3:]
        dim = len(spatial)
        ...
        # Encode theta latent with VAE
        theta_latent, metadata = self.encode_theta_latent(
            x, state_labels, 
            return_vae_details=return_vae_details, 
            use_vae=use_vae
        )

        metadata['theta_latent'] = theta_latent

        # Decode parameters (unchanged from original)
        ...
        # ODE integration (unchanged from original)
