...
class DISCOHouse(nn.Module):
    ...
    def __init__(self, 
        ...
        # Vector Quantizer for theta latents
        ...
        #self.quantizer = FSQ(levels = [10, 10, 10])
        #if quantizer_type == "fsq":
        #    self.quantizer = FSQ(levels = [10, 10, 10])
        
        #elif quantizer_type == "lfq":
        #    self.quantizer = LFQ(
        #        codebook_size = 1024,      # codebook size, must be a power of 2
        ...
        self.quantizer = SimVQ(theta_dim,
                               codebook_size,
                               rotation_trick=False,
                               frozen_codebook_dim=16)

        #self.layernorm = nn.LayerNorm(theta_dim)
    ...
    def encode_theta_latent(self, x: Tensor, state_labels: Tensor, return_vq_details: bool = True, use_vq: bool = True) -> Tensor:
        """Encode input x and state_labels to produce theta_latent with optional VQ details."""
        x_shape = x.shape
        B, T, C = x_shape[:3]
        spatial = x_shape[3:]
        dim = len(spatial)
        ...
        # new
        #theta_latent = self.layernorm(theta_latent)
        
        # Store pre-VQ codes
        theta_latent_pre_vq = theta_latent
        
        if use_vq:
            # Quantize the theta latent
            #theta_latent, indices, commit_loss = self.quantizer(theta_latent)

            (theta_latent, indices, vq_loss) = self.quantizer(theta_latent.unsqueeze(1))
            theta_latent = theta_latent.squeeze(1)
            metadata['vq_indices'] = indices
            metadata['vq_loss'] = vq_loss
            
            #theta_latent, indices = self.quantizer(theta_latent.unsqueeze(1))
            #theta_latent = theta_latent.squeeze(1)
            #metadata['vq_indices'] = indices

            #(theta_latent, indices, vq_loss), auxiliary_losses = self.quantizer(theta_latent.unsqueeze(1), return_loss_breakdown=True)
            #per_sample_entropy, codebook_entropy, commit_loss = auxiliary_losses
            #theta_latent = theta_latent.squeeze(1)
            #commit_loss = commit_loss.mean()
            
            #metadata['vq_indices'] = indices
            #metadata['vq_loss'] = vq_loss
            #metadata['per_sample_entropy'] = per_sample_entropy
            #metadata['codebook_entropy'] = codebook_entropy
            #metadata['commit_loss'] = commit_loss

        else:
            # No VQ - just pass through
            #metadata['commit_loss'] = torch.tensor(0.0, device=x.device)
            metadata['vq_indices'] = None
        
        if return_vq_details:
            metadata['theta_latent_pre_vq'] = theta_latent_pre_vq
    ...
    def forward(self, 
        ...
        theta_latent, metadata = self.encode_theta_latent(x, state_labels, return_vq_details=return_vq_details, use_vq=use_vq)
