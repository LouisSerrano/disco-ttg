...
class ResidualBlock(ConditionedBlock):
    ...
    def __init__(
        ...
        self.cond_emb = nn.Sequential(nn.LeakyReLU(0.1), nn.Linear(cond_channels, 2 * out_channels if use_scale_shift_norm else out_channels))
...
class Unet(nn.Module):
    ...
    def __init__(
        ...
        self.time_embed = nn.Sequential(nn.Linear(time_embed_dim, time_embed_dim), nn.GELU(), nn.Linear(time_embed_dim, time_embed_dim))
        ...
        for i in range(n_resolutions):
            ...
            if i < n_resolutions - 1:
                down.append(Downsample(in_channels, in_channels, n_dims=n_dims))
    ...
    def forward(self, x: torch.Tensor, time: torch.Tensor = None, z: torch.Tensor = None, h: list = None):
        ...
        emb = 0
        if time is not None:
            emb = emb + self.time_embed(fourier_embedding(time, self.time_embed_dim)) #[..., None]
