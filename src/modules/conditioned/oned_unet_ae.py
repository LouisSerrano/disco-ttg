...
class UpBlock(ConditionedBlock):
    ...
    def __init__(
        ...
        self.res = ResidualBlock(
            in_channels, # as we do not concatenate. + out_channels, 
            out_channels,
            cond_channels,
            activation=activation,
            norm=norm,
...
class MiddleBlock(ConditionedBlock):
    ...
    def __init__(
        ...
        self.compress = nn.Conv1d(n_channels, bottleneck_dim, 1, bias=False)
        self.expand = nn.Conv1d(bottleneck_dim, n_channels, 1, bias=False)
...
class Upsample(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int,  n_dims: int = 1):
        super().__init__()
        if n_dims == 1:
            self.conv = nn.ConvTranspose1d(in_channels, out_channels, 4, 2, 1)
        elif n_dims == 2:
            self.conv = nn.ConvTranspose2d(in_channels, out_channels, (4, 4), (2, 2), (1, 1))
        elif n_dims == 3:
            self.conv = nn.ConvTranspose3d(in_channels, out_channels, (4, 4, 4), (2, 2, 2), (1, 1, 1))
        else:
...
class Downsample(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, n_dims: int = 1):
        super().__init__()
        if n_dims == 1:
            self.conv = nn.Conv1d(in_channels, out_channels, 3, 2, 1)
        elif n_dims == 2:
            self.conv = nn.Conv2d(in_channels, out_channels, (3, 3), (2, 2), (1, 1))
        elif n_dims == 3:
            self.conv = nn.Conv3d(in_channels, out_channels, (3, 3, 3), (2, 2, 2), (1, 1, 1))
        else:
            raise ValueError(f"n_dims must be 1, 2, or 3, got {n_dims}")
...
class Unet(nn.Module):
    ...
    def __init__(
        ...
        if self.use_encoding:
            self.encoding_emb = nn.Sequential(nn.Linear(bottleneck_dim, time_embed_dim), self.activation, nn.Linear(time_embed_dim, time_embed_dim))
        ...
        for i in range(n_resolutions):
            ...
            if i < n_resolutions - 1:
                down.append(Downsample(in_channels+in_channels, in_channels, n_dims=n_dims))
        ...
        for i in reversed(range(n_resolutions)):
            ...
            if i > 0:
                up.append(Upsample(in_channels, in_channels, n_dims=n_dims))
    ...
    def forward(self, x: torch.Tensor, time: torch.Tensor = None, z: torch.Tensor = None, h: list = None):
        ...
        if not (time is None and z is None):
            emb = 0
            if time is not None:
        ...
        for m in self.down:
            ...
            if isinstance(m, Downsample):
                ...
                x = m(x, emb)
            new_h.append(x)
            #print('s down', s.shape, 'x', x.shape)
            #print('down', x.shape)
            #h.append(x)

        x, z = self.middle(x, emb)
        ...
        for i, m in enumerate(self.up):
            ...
            if isinstance(m, Upsample):
                ...
                # we do not do that here as it is an AutoEncoder -> Unet
                #
                # Get the skip connection from first half of U-Net and concatenate
                s = new_h.pop()
                #print('x', i, s.shape, x.shape)
                x = torch.cat((x, s), dim=1)
                #
                x = m(x, emb)
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        for i in range(n_resolutions):
            ...
            # Down sample at all resolutions except the last
            if i < n_resolutions - 1:
                down.append(Downsample(out_channels, out_channels, n_dims=n_dims))
        
        self.norm = nn.GroupNorm(8, out_channels)
        self.project = nn.Conv1d(out_channels, bottleneck_dim, 1)
        self.lift = nn.Conv1d(bottleneck_dim, out_channels, 1)
        ...
        for i in reversed(range(n_resolutions)):
            ...
            if i > 0:
                up.append(Upsample(in_channels, out_channels, n_dims=n_dims))
       
        self.norm_final = nn.GroupNorm(8, in_channels)
        # Combine the set of modules
        self.down = nn.ModuleList(down)
        self.up = nn.ModuleList(up)
    ...
    def forward(self, x: torch.Tensor, time: torch.Tensor = None, z: torch.Tensor = None):
        ...
        for m in self.down:
            ...
            #print('down', x.shape)
            #h.append(x)
        #print('final enc x', x.shape)
        #z = self.project(self.activation(self.norm(x)))
        z = self.project(self.activation(x))
        #z = self.project(x)
        x = self.activation(self.lift(z))
        ...
        x = self.final(self.activation(self.norm_final(x)))
        x = x.reshape(
            orig_shape[0], -1, (self.n_output_scalar_components + self.n_output_vector_components * 2), *orig_shape[3:]
        )
