...
import os
import sys
#sys.path.append(f'{os.environ["WORK"]}/multiple_physics_pretraining/models/')
try:
    from spatial_modules import hMLP_stem, hMLP_output, SubsampledLinear, hMLP_stem1d,  hMLP_output1d, AxialAttentionBlock, SpatialAttentionBlock1d
    from mixed_modules import build_spacetime_block, SpaceTimeBlock, SpaceTimeBlock1d
except:
    from .spatial_modules import hMLP_stem, hMLP_output, SubsampledLinear, hMLP_stem1d,  hMLP_output1d, AxialAttentionBlock, SpatialAttentionBlock1d
    from .mixed_modules import build_spacetime_block, SpaceTimeBlock, SpaceTimeBlock1d
...
class AViT(nn.Module):
    ...
    """
    def __init__(self, patch_size=(16, 16), embed_dim=768, processor_blocks=8, n_states=1,
                 override_block=None, drop_path=.2, small_compression=False):
        super().__init__()
        self.drop_path = drop_path
        self.dp = np.linspace(0, drop_path, processor_blocks)
        #self.space_bag = SubsampledLinear(n_states, embed_dim//4) # lol i don't care about this
        self.space_bag = nn.Linear(n_states, embed_dim//4)
        self.embed = hMLP_stem(patch_size=patch_size, in_chans=embed_dim//4, embed_dim=embed_dim, small_compression=small_compression)
        ...
        if override_block is not None:
            ...
            space_block = partial(AxialAttentionBlock, embed_dim)
            inner_block = partial(SpaceTimeBlock, hidden_dim=embed_dim, space_override=space_block)
            
        self.blocks = nn.ModuleList([inner_block(drop_path=self.dp[i])
                                     for i in range(processor_blocks)])
        self.debed = hMLP_output(patch_size=patch_size, embed_dim=embed_dim, out_chans=n_states, small_compression=small_compression)
...
class AViT1d(nn.Module):
    ...
    Args:
        ...
        n_states (int): Number of input state variables.  
    """
    def __init__(self, patch_size=(16, 16), embed_dim=512, processor_blocks=6, n_states=1,
        ...
        #self.space_bag = SubsampledLinear(n_states, embed_dim//4) # lol i don't care about this
        self.space_bag = nn.Linear(n_states, embed_dim//4)
        self.embed = hMLP_stem1d(patch_size=patch_size, in_chans=embed_dim//4, embed_dim=embed_dim)

        inner_block = partial(SpaceTimeBlock1d, hidden_dim=embed_dim)
            
        self.blocks = nn.ModuleList([inner_block(drop_path=self.dp[i])
                                     for i in range(processor_blocks)])
        self.debed = hMLP_output1d(patch_size=patch_size, embed_dim=embed_dim, out_chans=n_states)
    ...
    def expand_projections(self, expansion_amount):
        ...
        with torch.no_grad():
            ...
            self.space_bag = temp_space_bag
            # expand output projections
            out_head = nn.ConvTranspose1d(self.debed.embed_dim//4, self.debed.out_chans+expansion_amount, kernel_size=4, stride=4)
            temp_out_kernel = out_head.weight
            temp_out_bias = out_head.bias
            temp_out_kernel[:, :self.debed.out_chans, :, :] = self.debed.out_kernel
            temp_out_bias[:self.debed.out_chans] = self.debed.out_bias
    ...
    def forward(self, x):
        ...
        # Sparse proj
        #print('inp model', x.shape)
        x = rearrange(x, 't b c h -> t b h c')
        x = self.space_bag(x)
        #print('space bag', x.shape)

        #print('after space bag', x.shape)
        # Encode
        x = rearrange(x, 't b h c -> (t b) c h')
        x = self.embed(x)            
