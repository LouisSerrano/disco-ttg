...
class hMLP_stem(nn.Module):
    ...
    """
    def __init__(self, patch_size=(8,8), in_chans=3, embed_dim =768, small_compression=False):
        super().__init__()
        self.patch_size = patch_size
        self.in_chans = in_chans
        self.embed_dim = embed_dim
        if small_compression:
            self.in_proj = torch.nn.Sequential(
                *[nn.Conv2d(in_chans, embed_dim, kernel_size=2, stride=2, bias=False),
...
class hMLP_output(nn.Module):
    ...
    """
    def __init__(self, patch_size=(8,8), out_chans=3, embed_dim=768, small_compression=False):
        super().__init__()
        self.patch_size = patch_size
        self.out_chans = out_chans
        self.embed_dim = embed_dim
        self.small_compression = small_compression
        if small_compression:
            self.out_proj = nn.Sequential(*[nn.Conv2d(embed_dim, embed_dim//4, kernel_size=1),
                                            RMSInstanceNorm2d(embed_dim//4, affine=True),
                                            nn.GELU()])
        ...
        out_head = nn.ConvTranspose2d(embed_dim//4, out_chans, kernel_size=2 if small_compression else 4, stride=2 if small_compression else 4)
    ...
    def forward(self, x):
        ...
        x = F.conv_transpose2d(x, self.out_kernel, self.out_bias, stride=2 if self.small_compression else 4)
        return x
...
class SpatialAttentionBlock1d(nn.Module):
    def __init__(self, hidden_dim=768, num_heads=12, drop_path=0, layer_scale_init_value=1e-6, bias_type='rel'):
        ...
        self.gamma = nn.Parameter(layer_scale_init_value * torch.ones((hidden_dim)), 
                            requires_grad=True) if layer_scale_init_value > 0 else None
        self.input_head = nn.Conv1d(hidden_dim, 3*hidden_dim, 1)
        self.output_head = nn.Conv1d(hidden_dim, hidden_dim, 1)
        self.qnorm = nn.LayerNorm(hidden_dim//num_heads)
        self.knorm = nn.LayerNorm(hidden_dim//num_heads)
        if bias_type == 'none':
            self.rel_pos_bias = lambda x, y: None
    ...
    def forward(self, x):
        ...
        # Rearrange for attention on y-axis (height)
        x = rearrange(x, 'b (he c) h -> b he h c', he=self.num_heads)
        q, k, v = x.tensor_split(3, dim=-1)
        q, k = self.qnorm(q), self.knorm(k)
        rel_pos_bias = self.rel_pos_bias(H, H)
        ...
        x = rearrange(x, 'b he h c -> b (he c) h')
        x = self.norm2(x)
        x = self.output_head(x)
        #x = rearrange(x, '(t b) c h -> t b c h', t=T)
        output = self.drop_path(x * self.gamma[None, :, None]) + input
        return output
