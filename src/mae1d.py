...
class PatchEmbed1D(nn.Module):
    ...
    def __init__(self, grid_size=256, patch_size=16, in_chans=3, history=16, embed_dim=768):
        super().__init__()
...
class MaskedAutoencoderViT(nn.Module):
    """ Masked Autoencoder with VisionTransformer backbone for 1D spatial + temporal data with channels
    """
    def __init__(self, grid_size=256, patch_size=16, in_chans=3, history=16,
        ...
        self.decoder_pred = nn.Linear(decoder_embed_dim, patch_size * in_chans * history, bias=True) # decoder to patch
        # --------------------------------------------------------------------------
    ...
    def initialize_weights(self):
        ...
        # initialize patch_embed projection layer
    ...
    def patchify(self, x):
        """
        x: (N, C, H, T)
        output: (N, num_patches, patch_size * in_chans * history)
        """
        B, C, H, T = x.shape
        p = self.patch_size
        assert H % p == 0
        ...
        num_patches = H // p
        # Reshape to patches and stack channel and temporal components
        x = x.reshape(B, C, num_patches, p, T)
        x = x.permute(0, 2, 3, 1, 4)  # (B, num_patches, patch_size, in_chans, history)
        x = x.reshape(B, num_patches, p * C * T)
        return x

    def unpatchify(self, x):
        """
        x: (N, num_patches, patch_size * in_chans * history)
        output: (N, C, H, T)
        """
        ...
        C = self.in_chans
        T = self.history
        assert patch_dim == p * C * T
        
        # Reshape patches back to spatial-temporal-channel structure
        x = x.reshape(B, num_patches, p, C, T)
        x = x.permute(0, 3, 1, 2, 4)  # (B, in_chans, num_patches, patch_size, history)
        x = x.reshape(B, C, num_patches * p, T)
        return x
    ...
    def forward_loss(self, x, pred, mask):
        """
        x: [N, C, H, T]
        pred: [N, num_patches, patch_size * in_chans * history]
    ...
    def forward(self, x, mask_ratio=0.75):
        ...
        pred = self.forward_decoder(latent, ids_restore)  # [N, num_patches, patch_size * in_chans * history]
