"""
Latent Splitting Fine-tuning Script for Neural Operator Splitting

This script implements a two-phase latent theta optimization approach for neural operator splitting:
1. Phase 1: Optimize latent theta parameters (theta_latent) with fixed alpha weights
2. Phase 2: Fine-tune decoded theta parameters with preservation loss

This approach differs from symbolic regression by directly optimizing latent representations
before fine-tuning the actual neural network parameters.
"""
...
def apply_operator_splitting(x, state_labels, operators1, operators2):
    """
    Apply two-operator splitting by sequentially applying operators1 then operators2.
    """
    # Apply first operator with uniform weights (no alpha weighting)
    outputs1 = operators1(x, state_labels)
    intermediate = outputs1.mean(dim=0)  # Average outputs from first operator group
    
    # Apply second operator with uniform weights
    outputs2 = operators2(intermediate, state_labels)
    return outputs2.mean(dim=0)  # Average outputs from second operator group


def solve_splitting_ode(x_input, operators1, operators2, state_labels, 
                       integration_time=1.0, n_future_steps=1, 
                       solver='rk4', rtol=1e-7):
    """
    Solve neural ODE using operator splitting.
    """
    # Create ODE function for operator splitting
    def ode_func(t, x):
        return apply_operator_splitting(x, state_labels, operators1, operators2)
...
class NeuralOperatorSplittingExperiment:
    ...
    def phase1_latent_theta_optimization(self, inp, target, theta_latent, state_labels) -> Tuple[torch.Tensor, Dict]:
        """
        Phase 1: Optimize latent theta parameters for two-operator splitting.
        
        Returns:
            Optimized latent theta and training history
        """
        print("=" * 60)
        print("PHASE 1: LATENT THETA OPTIMIZATION")
        print("=" * 60)
        
        # Parameters
        epochs = self.config.get('phase1_epochs', 500)
        num_operators = theta_latent.shape[0]
        training_horizon = 1
        lambd = self.config.get('sparsity_coeff', 1e-2)
        
        # Initialize learnable latent theta
        theta_latent_ = (torch.zeros_like(theta_latent)).clone().detach().requires_grad_(True)
        
        # Optimizer and scheduler
        optimizer = torch.optim.AdamW([theta_latent_], lr=1e-1, weight_decay=0)
        ...
        for epoch in tqdm(range(epochs), desc="Phase 1 Training"):
            ...
            # Decode theta from latent
            theta_decoded = self.model.decode_theta(theta_latent_, dim=1)
            
            # Get batched operators - split them into two groups
            operators1 = get_batched_operators(theta_decoded[:num_operators//2], self.model, dim=1)
            operators2 = get_batched_operators(theta_decoded[num_operators//2:], self.model, dim=1)
            
            # Forward pass with operator splitting
            pred = []
            current = x_train
            for _ in range(training_horizon):
                current = solve_splitting_ode(
                    current, operators1, operators2, state_labels,
                    integration_time=1, n_future_steps=1)[-1]
                pred.append(current)
            
            pred = torch.cat([p.unsqueeze(0) for p in pred], axis=0)
            
            # Calculate losses
            rel_loss = self.relative_l2_error(pred, y_train)
            sparsity_loss = theta_latent_.abs().sum(-1).mean()  # Regularize latent space
            ...
            if epoch % 50 == 0 or epoch == epochs - 1:
                ...
                history['theta_latent_norm'].append(torch.norm(theta_latent_).item())
                
                print(f"Epoch [{epoch+1}/{epochs}] | "
                      f"Train Loss: {rel_loss.item():.6f} | "
                      f"Sparsity Loss: {sparsity_loss.item():.6f} | "
                      f"Theta Norm: {torch.norm(theta_latent_).item():.6f}")
        
        print(f"Phase 1 completed. Latent theta norm: {torch.norm(theta_latent_).item():.6f}")
        return theta_latent_.detach(), history
    
    def phase2_theta_finetuning(self, inp, target, theta_latent_opt, state_labels,
                               operator_inp, operator_target) -> Tuple[torch.Tensor, Dict]:
        """
        Phase 2: Fine-tune decoded theta parameters with multi-step prediction.
        
        Returns:
            Optimized theta and training history
        """
        print("=" * 60)
        print("PHASE 2: THETA PARAMETER FINE-TUNING")
        print("=" * 60)
        
        # Parameters
        epochs = self.config.get('phase2_epochs', 500)
        preservation_coeff = 0  # Disabled for now
        noise_level = 1e-3
        
        # Initialize parameters - decode theta from optimized latent
        with torch.no_grad():
            theta_decoded = self.model.decode_theta(theta_latent_opt, dim=1)
        
        theta_ = theta_decoded.clone().detach().requires_grad_(True)
        
        # Optimizer
        optimizer = torch.optim.AdamW([theta_], lr=1e-4, weight_decay=0)
        ...
        for epoch in tqdm(range(epochs), desc="Phase 2 Training"):
            ...
            # Test data forward pass: operator splitting
            ...
            pred_test = torch.cat([p.unsqueeze(0) for p in pred], axis=0)
            ...
            loss = test_loss + preservation_coeff * preservation_loss
        ...
        return theta_.detach(), history
    
    def evaluate_model(self, inp, target, theta, state_labels) -> Dict:
    ...
    def run_experiment(self) -> Dict:
        ...
        # Get initial latent theta by encoding the structured operators
        with torch.no_grad():
            theta_latent, metadata = self.model.encode_theta_latent(operator_inp, state_labels)
            # Average theta_latent if we have multiple trajectories per operator
            if n_trajectories_per_operator > 1:
                total_operators = 2 * K
                theta_latent = theta_latent.view(n_trajectories_per_operator, total_operators, -1).mean(dim=0)
        
        # Phase 1: Latent theta optimization on test data
        theta_latent_opt, phase1_history = self.phase1_latent_theta_optimization(
            inp, target, theta_latent, state_labels
        )
        
        # Phase 2: Theta parameter fine-tuning with preservation
        theta_opt, phase2_history = self.phase2_theta_finetuning(
            inp, target, theta_latent_opt, state_labels, 
            operator_inp, operator_target
        )
        
        # Evaluation on test data
        eval_results = self.evaluate_model(inp, target, theta_opt, state_labels)
...
def create_plots(results: Dict, output_dir: str):
    ...
    # Plot 2: Best operators information
...
def main():
    ...
    parser.add_argument('--rel-loss-coeff', type=float, default=1,
                        help='Coefficient for preservation loss (default: 1)')
    ...
    if args.num_runs == 1:
        ...
        phase1_errors = [min(r['phase1_history']['train_loss']) for r in all_results]
        phase2_errors = [min(r['phase2_history']['train_loss']) for r in all_results]
        final_errors = [r['evaluation']['error'] for r in all_results]
        
        stats = {
            'num_runs': args.num_runs,
            'phase1_error': {
                'mean': np.mean(phase1_errors),
                'std': np.std(phase1_errors),
                'min': np.min(phase1_errors),
                'max': np.max(phase1_errors)
            },
            'phase2_error': {
                'mean': np.mean(phase2_errors),
                'std': np.std(phase2_errors),
                'min': np.min(phase2_errors),
                'max': np.max(phase2_errors)
            },
            'final_error': {
            ...
            'improvement_phase1_to_final': {
                'mean': np.mean([p1/f for p1, f in zip(phase1_errors, final_errors)]),
                'std': np.std([p1/f for p1, f in zip(phase1_errors, final_errors)]),
                'min': np.min([p1/f for p1, f in zip(phase1_errors, final_errors)]),
                'max': np.max([p1/f for p1, f in zip(phase1_errors, final_errors)])
            }
        }
        ...
        print(f"  Phase 1 Error:   {stats['phase1_error']['mean']:.6f} ± {stats['phase1_error']['std']:.6f}")
        print(f"  Phase 2 Error:   {stats['phase2_error']['mean']:.6f} ± {stats['phase2_error']['std']:.6f}")
        print(f"  Final Error:     {stats['final_error']['mean']:.6f} ± {stats['final_error']['std']:.6f}")
        print(f"  Improvement (Phase1→Final): {stats['improvement_phase1_to_final']['mean']:.2f} ± {stats['improvement_phase1_to_final']['std']:.2f}")
