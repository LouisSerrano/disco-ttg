...
class NeuralOperatorSplittingExperiment:
    ...
    def encode_structured_operators(self, K=4, n_trajectories_per_operator=1, v_range=(0.1, 1.0), D_range=(0.1, 1.0)) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, list]:
        """
        ...
        Returns:
            ...
            theta_latent: [2*K, latent_dim] tensor of latent encodings
        ...
        return theta, theta_latent, all_inp_trajectories, all_target_trajectories, unique_params
    ...
    def finetune_theta_with_composition(self, inp, target, theta, theta_latent, best_composition, state_labels, 
                                       operator_inp, operator_target, optimize_latent=False) -> Tuple[torch.Tensor, torch.Tensor, Dict]:
        """
        ...
        for epoch in tqdm(range(epochs), desc="Finetuning"):
            ...
            # Preservation loss: ensure operators maintain their original capabilities
            preservation_loss = 0.0
            if preservation_coeff > 0:
                # Sample from operator trajectories
                t2 = random.randint(0, self.n_input_frames - training_horizon - 1)
                
                # Only evaluate preservation for operators that we're finetuning
                for i, op_idx in enumerate(best_composition):
                    if op_idx < operator_inp.shape[0]:  # Make sure we have data for this operator
                        x_op = operator_inp[op_idx:op_idx+1, t2].to(self.device)
                        x_op = x_op + torch.randn_like(x_op) * noise_level
                        y_op = operator_inp[op_idx:op_idx+1, t2+1:t2+1+training_horizon].to(self.device)
                        y_op = rearrange(y_op, "b t c h -> t b c h")
                        
                        # Single operator prediction
                        theta_idx = idx_to_theta[op_idx]
                        single_operator = get_batched_operators(current_theta[theta_idx:theta_idx+1], self.model, dim=1)
                        pred_single = solve_single_operator_ode(
                            x_op.squeeze(0), single_operator, state_labels,
                            integration_time=training_horizon, n_future_steps=training_horizon
                        )
                        
                        preservation_loss += self.relative_l2_error(pred_single, y_op)
                
                if len(best_composition) > 0:
                    preservation_loss /= len([i for i in best_composition if i < operator_inp.shape[0]])
    ...
    def run_experiment(self) -> Dict:
        ...
        theta, theta_latent, operator_inp, operator_target, operator_params = self.encode_structured_operators(
        ...
        if self.config.get('enable_finetuning', True) and len(best_composition) > 0:
            ...
            theta_finetuned, theta_latent_finetuned, finetune_history = self.finetune_theta_with_composition(
                inp, target, theta, theta_latent, best_composition, state_labels, operator_inp, operator_target, optimize_latent
            )
        else:
...
def main():
    ...
    parser.add_argument('--preservation-coeff', type=float, default=0.0,
