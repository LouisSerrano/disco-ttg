...
def fine_tune_theta_latent(model, theta_latent, x_test, y_test, state_labels, dim, 
    ...
    if optimizer_type.lower() == "sgd":
        optimizer = torch.optim.SGD([theta_latent1], lr=lr, weight_decay=0)
    elif optimizer_type.lower() == "sgd_momentum":
        optimizer = torch.optim.SGD([theta_latent1], lr=lr, momentum=momentum, weight_decay=0)
    elif optimizer_type.lower() == "adam":
        optimizer = torch.optim.Adam([theta_latent1], lr=lr, weight_decay=0)
    else:
    ...
    for epoch in tqdm(range(epochs), desc="Fine-tuning"):
        ...
        # Forward pass with finer integration if num_steps > 1
        if num_steps == 1:
            pred, _ = model.solve_ode(
                        x_test[:, t], theta1, state_labels, dim, 
                        n_future_steps=training_horizon, integration_time=1, predict_normed=False, metadata={}
                    )
        else:
            pred = []
            ...
            with torch.no_grad():
                ...
                if num_steps == 1:
                    pred_test, _ = model.solve_ode(
                        ...
                        n_future_steps=n_output_frames, integration_time=1, predict_normed=False, metadata={}
                    )
                else:
                    pred_test = []
                    for _ in range(n_output_frames):
                        ...
                        for _ in range(num_steps):
                            ...
                            current_state = pred_step[:, -1]
                        pred_test.append(current_state)
                        x_test_ = current_state
...
def get_data(
    ...
    # finetune the model
    rollout_error = fine_tune_theta_latent(model, theta_latent, inp, target[:, :n_output_frames], state_labels, dim, n_input_frames=n_input_frames, n_output_frames=n_output_frames, epochs=epochs, device=device, num_steps=num_steps)
...
def main(cfg: DictConfig):
    ...
    run_composition_tests(model, device, cfg, n_points=n_points, output_csv=f"{output_dir}/single_finetune_test_results_{n_points}.csv", num_steps=num_steps)
