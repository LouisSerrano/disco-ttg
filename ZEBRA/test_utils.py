...
def get_test_metrics_1d(loader, model, input_size=16, output_size=34, num_examples=0, vocab_size=256):
    ...
    with torch.no_grad():
        for images in loader:
            ...
            # Tokenize images
            x = rearrange(images[..., :input_size], "b c h t -> (b t) c h").float()
            codes, indices = model.tokenizer(x, return_codes=True)
            tokens = rearrange(indices, '(b t) c h -> b c h t', t=input_size)
            h_lat = tokens.shape[2]

            #print('tokens', tokens.shape)

            sequences, _ = model.model.prepare_sequence(tokens, None)
            # we get rid of the last eot_token, and eos_token
            sequences = sequences[:, :-2]

            # Generate predictions
            rollout_len = output_size
            #print('rollout_len', rollout_len)
            frame_size = h_lat*tokens.shape[1]
            #print('frame_size', frame_size)
            total_output_len = rollout_len*frame_size
            #print('total_len', total_output_len)
            max_length = sequences.shape[1] + total_output_len
            ...
            # Decode predictions
            indices = rearrange(output[:, -total_output_len:], 'b (t h c) -> (b h t) c', 
            ...
            #print('quantized_pred 0', quantized_pred.shape)
            quantized_pred = rearrange(quantized_pred, '(b h t) c -> (b t) c h', t=rollout_len, h=h_lat)

            #print('quantized_pred 1', quantized_pred.shape)
            upred = model.tokenizer.decode(quantized_pred)
            #print('quantized_pred 2', quantized_pred.shape)
            upred = rearrange(upred, '(b t) c h -> b c h t', t=rollout_len)

            urec = model.tokenizer.decode(codes)
...
def get_test_metrics_2d(loader, model, input_size=16, output_size=32, max_len_size=31, num_examples=0, vocab_size=1024):
    ...
    with torch.no_grad():
        for images in loader:
            ...
            sequences, _ = model.model.prepare_sequence(sequences, None)
            # we get rid of the last eot_token, and eos_token
            sequences = sequences[:, :-2]
            ...
            #print(frame_size, rollout_len, h, w, max_length)
            ...
            for i in range(rollout_len):
                ...
                # Just add past_key_values parameter
                ...
                output_tokens.append(sequences[:, -frame_size:])
        
                if sequences.shape[1] >= max_len_size*frame_size+2:
                    sequences, _ = model.model.prepare_sequence(sequences[:, -chunk_size*frame_size:], None)
                    past_key_values = None  # Reset cache after slicing
